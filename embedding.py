...
def create_adj_avg_new(adj_cur):
    '''
    create adjacency
    '''
    deg = np.sum(adj_cur, axis=1)
    ...
    deg = 1 / deg
    deg_mat = np.diag(deg)
    adj_cur = adj_cur.dot(deg_mat.T).Tx

    return adj_cur


def createWlEmbedding_new(node_features, adj_mat, h):
    graph_feat = []
    D_matrix = cdist(node_features, node_features)
    adj = adj_mat.A
    for it in range(h+1):
        if it == 0:
            graph_feat.append(node_features)
        else:
            adj_cur = np.multiply(adj, D_matrix)
            adj_cur = create_adj_avg_new(adj_cur)
            graph_feat_cur = 0.5 * \
                (np.dot(adj_cur, graph_feat[it-1]) + graph_feat[it-1])
            graph_feat.append(graph_feat_cur)
    result = np.mean(np.concatenate(graph_feat, axis=1), axis=0).reshape(1, -1)
    return result
